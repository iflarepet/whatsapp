...
if (!empty($_FILES['file']) && $_FILES['file']['error'] == UPLOAD_ERR_OK) {
    ...
    if (in_array($ext, $allow)) {
        ...
        redirect($base_url . "pages/group_nomor.php");
        exit;
    }
    ...
    if ($xlsx = SimpleXLSX::parse($file)) {
        ...
        foreach ($xlsx->rows() as $elt) {
            if ($i >= $a) {
                ...
                $id_nomor = getIdnomor($nomor);

                $q = mysqli_query($koneksi, "INSERT INTO detail_group(`id_group`,`id_nomor`) VALUES('$id_group','$id_nomor')");
            }
            $i++;
        }
    } else {
